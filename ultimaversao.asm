...
testa_E:
    MOV R2, 3
	MOV [JOGO], R2               ; desbloqueia processo ...............? (qualquer registo serve)
    MOV R1, FUNDO_JOGO_TERMINADO        ; cenário de fundo número 0
	MOV [SELECIONA_CENARIO_FUNDO], R1 ; seleciona o cenário de fundo
    JMP obtem_tecla
...
colisao_meteoro_bom:
    ...
    JMP fim_colisao_rover

colisao_meteoro_mau:          ;há de ser game over
	MOV R2, 3
	MOV [JOGO], R2
    MOV R2, FUNDO_GAME_OVER_COLISAO       ; cenário de fundo número 0 & som numero 0 - o numero do ecra e do som é o mesmo
...
colisao_disparo:
	...
	MOV R11, DEF_METEOROS_MAUS   ; tabela com os niveis dos meteoros maus
	CMP R10, R11                 ; verifica qual é o tipo de meteoro
	JNZ obtem_altura_meteoro_bom ; colidiu com um meteoro bom
    MOV [acerta_nave_ma], R1
...
game_over:
    MOV R2, 3
	MOV [JOGO], R2
    MOV R2, FUNDO_GAME_OVER_ENERGIA       ; cenário de fundo número 0 & som numero 0 - o numero do ecra e do som é o mesmo
	MOV [SELECIONA_CENARIO_FUNDO], R2 ; seleciona o cenário de fundo
